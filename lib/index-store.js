...
function getDb() {
  ...
  try {
    ...
    // Verify the DB actually has the artists table (empty DB = no tables)
    const row = _db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='artists'").get()
    _dbHasTables = !!row
  } catch (e) {
    console.warn('index-store: failed to open SQLite DB:', e.message)
    _db = null
  }
  return _db
}
  ...
  try {
    ...
    for (const handle of handles) {
      ...
      try {
        ...
        changed++
      } catch (e) { console.warn(`index-store: failed to parse site.json for ${handle}:`, e.message) }
    }
