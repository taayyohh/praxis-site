...
// --- Journal ---
export const JournalKeySchema = z.object({
  key: z.string({ error: 'invalid key' }).regex(/^[0-9a-fA-F]{64}$/, 'invalid key'),
})

export const JournalPostSchema = z.object({
  filename: z.string({ error: 'missing filename or content' }).min(1, 'missing filename or content'),
...
export const BlogPostSchema = z.object({
  filename: z.string({ error: 'filename required' }).min(1, 'filename required'),
  content: z.string({ error: 'content required' }).min(1, 'content required'),
})
