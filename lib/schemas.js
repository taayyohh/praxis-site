...
export const WalletStoreSchema = z.object({
  ...
  // Optional signed challenge `praxis-store:<addr>:<ts>`. Required on
  // first-write (no existing shared blob) so a bystander can't squat
  // the slot with a payload keyed by an address they don't hold. On
  // overwrite the session-token path still applies.
