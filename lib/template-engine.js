...
${myArtistsSection}
<div id="feed" data-blog="${blogRegistryAddr}" data-owner="${siteData.wallet || ''}">
  <p id="feed-status" style="color:var(--muted)"><span class="praxis-loader"></span></p>
  <div id="feed-posts"></div>
</div>
<div id="supporter-collection-section" data-wallet="${siteData.wallet || ''}" style="display:none">
  ...
  <div id="masonry-grid" class="masonry-grid"></div>
      ...
      indexContent = fill(tpl.index, {
        name: siteData.name,
        bio: siteData.bio || siteData.shortBio || '',
        shortBio: siteData.shortBio || siteData.bio || '',
        highlights: highlightsHtml,
        cv: cvHtml,
        blogRegistry: blogRegistryAddr,
        wallet: siteData.wallet || '',
        representation: siteData.representation || '',
  ...
  for (const moduleEntry of getEnabledModules(siteData)) {
    ...
    if (page === route) {
      ...
      return renderSectionPage(tpl, route, content, label, i18nKey, siteData)
    }
  }
