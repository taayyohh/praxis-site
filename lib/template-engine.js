...
function wrapHtml(tpl, content, title, canonicalPath, ogImage, site, opts = {}) {
  ...
  return fill(tpl.layout, {
    ...
    wallet: escapeForHtml(site.wallet || ''),
    ...
    themeFont: (site.theme?.font || "-apple-system, 'Helvetica Neue', Arial, sans-serif").replace(/[;{}()\\<>]/g, ''),
  })
}
...
<p class="library-caption" style="color:var(--muted);max-width:60ch;margin:0 0 1.5em;line-height:1.5">a community-curated repository of free and open knowledge. add pdfs, essays, and articles you have the rights to share.</p>
<p id="library-status"><span class="praxis-loader"></span></p>
<div id="library-add-form" style="margin-bottom:2em"></div>
<div id="library-list"></div>
<p class="library-legal" style="color:var(--dim);font-size:0.8em;margin-top:3em;padding-top:1em;border-top:1px solid var(--border)">library items are contributed by network members. see the <a href="https://ourpraxis.network/dmca" style="color:var(--muted)">copyright policy</a> for how to report material and how takedowns work.</p>
</div>`
}
  ...
  <div id="messages-sidebar" class="messages-sidebar">
    <div class="messages-sidebar-header" style="display:flex;align-items:center;justify-content:space-between">
      ...
      <div style="display:flex;align-items:center;gap:0.25em;margin-left:auto">
        <button id="messages-compose" style="background:none;border:none;color:var(--fg);cursor:pointer;padding:0;font-family:inherit;display:inline-flex;align-items:center;justify-content:center;-webkit-tap-highlight-color:transparent"><i class="ph ph-plus-circle" style="font-size:1.2em"></i></button>
        <button id="messages-settings-btn" style="background:none;border:none;color:var(--muted);cursor:pointer;padding:0;display:inline-flex;align-items:center;justify-content:center;-webkit-tap-highlight-color:transparent"><i class="ph ph-gear" style="font-size:1.2em"></i></button>
      </div>
    </div>
...
export function renderPage(templateName, pageName, siteData, rootDir) {
  ...
  <p class="bio">${escapeForHtml(siteData.bio || '')}</p>
...
<div id="supporter-collection-section" data-wallet="${siteData.wallet || ''}">
  ...
  if (page.startsWith('blog/')) {
    ...
    const blogCollectionsJson = JSON.stringify(blogCollections).replace(/</g, '\\u003c')
    const blogPostCollectionsJson = JSON.stringify(blogPostCollections).replace(/</g, '\\u003c')
    const colDataScript = `<script>window.__blogCollections=${blogCollectionsJson};window.__blogPostCollections=${blogPostCollectionsJson};window.__blogActiveCollection=${JSON.stringify(colSlug).replace(/</g, '\\u003c')};</script>`
