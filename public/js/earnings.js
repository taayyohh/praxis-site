...
function buildHistory(earned, contributed, resolve, ethPrices) {
  const items = [
    ...earned.mediaSales.map(s => ({ ...s, label: t('earnings.mediaSale'), detail: escapeHtml(s.title), sign: '+' })),
    ...earned.projectItems.map(p => ({ ...p, label: t('earnings.projectEarning'), detail: escapeHtml(p.title), sign: '+' })),
    ...contributed.fundingItems.map(f => ({ ...f, label: t('earnings.funded'), detail: escapeHtml(f.title), sign: '-' })),
    ...contributed.purchaseItems.map(p => ({ ...p, label: t('earnings.purchased'), detail: escapeHtml(p.title), sign: '-' })),
  ]
...
function renderHistoryItems(items, ethPrices) {
  return items.map(h => {
    ...
    return `<div style="display:flex;align-items:center;gap:1ch;padding:0.5em 0;border-bottom:1px solid var(--border)">
      ...
      <span style="color:${color};white-space:nowrap">${formatPriceSync(h.amount, ethPrices)}</span>
...
function renderEarnings(el, unclaimed, earned, contributed, addr, mediaAddr, ticketUnclaimed = 0n, ethPrices = null) {
  ...
  if (totalUnclaimed > 0n) {
    ...
    if (unclaimed.media > 0n) {
      html += `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5em">
        <span>${formatPriceSync(unclaimed.media, ethPrices)} <span style="color:var(--dim)">${t('earnings.fromMedia')}</span></span>
    ...
    if (unclaimed.praxis > 0n) {
      html += `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5em">
        <span>${formatPriceSync(unclaimed.praxis, ethPrices)} <span style="color:var(--dim)">${t('earnings.fromProjects')}</span></span>
    ...
    if (ticketUnclaimed > 0n) {
      html += `<div style="display:flex;justify-content:space-between;align-items:center">
        <span>${formatPriceSync(ticketUnclaimed, ethPrices)} <span style="color:var(--dim)">${t('earnings.fromTickets') || 'from ticket sales'}</span></span>
  ...
  html += `<div><span style="font-size:1.2em">${formatPriceSync(totalEarned, ethPrices)}</span> <span style="color:var(--dim)">${t('earnings.earned')}</span></div>`
  html += `<div><span style="font-size:1.2em">${formatPriceSync(totalContributed, ethPrices)}</span> <span style="color:var(--dim)">${t('earnings.contributed')}</span></div>`
  html += `</div>`

  // breakdown grid
  html += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:1em 3em;margin-bottom:2em;font-size:0.9em">`
  html += `<div>${formatPriceSync(earned.mediaTotal, ethPrices)} <span style="color:var(--dim)">${t('earnings.media')}</span></div>`
  html += `<div>${formatPriceSync(contributed.fundingTotal, ethPrices)} <span style="color:var(--dim)">${t('earnings.projectsFunded')}</span></div>`
  html += `<div>${formatPriceSync(earned.projectEarnings + unclaimed.praxis, ethPrices)} <span style="color:var(--dim)">${t('earnings.projects')}</span></div>`
  html += `<div>${formatPriceSync(contributed.purchaseTotal, ethPrices)} <span style="color:var(--dim)">${t('earnings.mediaPurchased')}</span></div>`
  html += `</div>`
  ...
  if (_allHistory.length > 0) {
    ...
    html += `<div id="earnings-history">${renderHistoryItems(firstPage, ethPrices)}</div>`
    if (_allHistory.length > HISTORY_PAGE_SIZE) {
      html += `<button id="earnings-load-more" class="buy-btn" style="width:100%;margin-top:1em">${t('earnings.loadMore') || 'load more'}</button>`
    }
  } else {
  ...
  // load more history
  document.getElementById('earnings-load-more')?.addEventListener('click', () => {
    const historyEl = document.getElementById('earnings-history')
    const btn = document.getElementById('earnings-load-more')
    ...
    if (_historyShown >= _allHistory.length) btn.style.display = 'none'
  })
