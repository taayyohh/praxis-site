...
async function loadConversations() {
  ...
  try {
    ...
    dbg(`praxis: rendering ${displayItems.length} conversations, ${filtered.length} direct / ${requests.length} requests`)

    let html = ''

    if (displayItems.length === 0) {
      html += `<div id="messages-syncing" style="color:var(--muted);padding:2em 1em;text-align:center;font-size:0.9em">no conversations yet</div>`
    } else {
