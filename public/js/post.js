...
async function initPost() {
  ...
  try {
    ...
    // by someone else can't rewrite this post's rendered content. Both
    // sides must be truthy — null==null would otherwise fold a malformed
    // response into the display.
    ...
    const amendments = rawAmendments.filter(a => {
      const aAuth = String(a.author || '').toLowerCase()
      return parentAuthor && aAuth && aAuth === parentAuthor
    })
  ...
  function renderPostItem(p) {
    ...
    if (imgMatch) {
      ...
      thumbHtml = `<div style="flex-shrink:0;width:100px;height:80px;margin-left:1em;border-radius:4px;overflow:hidden;background:#000;position:relative;display:flex;align-items:center;justify-content:center">
        <video src="${escapeHtml(vidMatch[1])}" preload="metadata" playsinline muted style="width:100%;height:100%;object-fit:cover;pointer-events:none"></video>
    ...
    let mediaTopHtml = ''
    if (vidMatch) {
      mediaTopHtml = `<div class="feed-media-wrap"><div class="video-lazy" data-src="${escapeHtml(vidMatch[1])}" style="position:relative;width:100%;aspect-ratio:16/9;overflow:hidden;background:#000;display:flex;align-items:center;justify-content:center"><video src="${escapeHtml(vidMatch[1])}" preload="metadata" playsinline muted style="width:100%;height:100%;object-fit:cover;pointer-events:none"></video></div></div>`
    } else if (imgMatch) {
