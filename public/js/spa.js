...
async function loadRouteModules(pathname) {
  ...
  if (!modules) {
    ...
    // Project/event vanity: /project/<slug>, /event/<slug>
    if ((seg === 'project' || seg === 'event') && path.indexOf('/', 2) > 0) modules = ROUTE_MODULES['/' + seg] || ROUTE_MODULES['/project']
  }
  if (!modules) return
  const loads = []
